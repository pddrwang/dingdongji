...
`.docx` 手稿点击后在**当前面板的小窗口**内免费预览：优先用本机 `mammoth`（`~/ddj/venvs/docx-render`）把 docx 转成语义 HTML（图片内联），没有时回退 macOS `textutil` 的 HTML，再回退纯文本；docx HTML 经白名单清洗后再渲染。需要真正的 Word 版式时可点「外部查看器打开」（可选，取决于你安装的第三方查看器，可能收费）。Markdown/文本仍用面板内文本预览，PDF 使用本机提取器或 PDF 预览。

“投稿管理”用于准备材料，不会自动向期刊提交稿件。文本与 HTML 预览都不等于完整版式或最终排版校验。
